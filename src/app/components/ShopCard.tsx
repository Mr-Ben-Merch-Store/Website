...
export default function ShopCard() {
    ...
    var products_load = p.map((item, index) =>
        item.map((item1, index1) => (
            <div
                className="max-w-sm max-h-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 m-10 justify-items-center"
                key={index}
            >
                <h2>{item1.name}</h2>
